...
public class DeleteDocArchiveToDelete {
	private WebDriver driver;
	private WebElement table;
	private List<WebElement> table_row;
	private List<WebElement> Doc_List;
	private String[] table_data = new String[5];
	private String[] Updated_table_data = new String[5];
	private int Task_no = 0;
	private boolean File_Match = false;
	private boolean Audit_Match = false;
	private boolean Doc_Match = false;
	private boolean Meta_Match = false;
...
driver.manage().timeouts().implicitlyWait(20, TimeUnit.SECONDS);
	  ...
	  driver.findElement(By.xpath("//*[contains(text(), 'Documents List')]")).click();
	  driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	  
	  Doc_List = driver.findElement(By.id("ctl00_cp1_tbl")).findElements(By.tagName("tr"));
	  System.out.println("There is/are " + (Doc_List.size() - 1) + " Document/s on the lists." );
	  if (Doc_List.size() > 1){
		  Doc_ID = Doc_List.get(1).findElements(By.tagName("td")).get(0).getText();
		  System.out.println("Document ID is " + Doc_ID);
		  
		  //Click view button on the document
		  driver.findElement(By.id("ctl00_cp1_lnkDCID|" + Doc_ID)).click();
		  driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		  ...
		  //Verify the status change to delete by sender
		  Doc_List = driver.findElement(By.id("ctl00_cp1_tbl")).findElements(By.tagName("tr"));
		  System.out.println(Doc_List.get(1).findElements(By.tagName("td")).get(4).getText());
		  Assert.assertTrue(Doc_List.get(1).findElements(By.tagName("td")).get(4).getText().equals("Deleted By Sender"));
	  ...
	  //Assert.assertTrue(table_row.get(6).findElements(By.tagName("td")).get(1).getText().equals("1"));
	  ...
	  Audit_File_Name = Doc_ID + "_Audit.csv";
	  System.out.println("Audit_File_Name is " + Audit_File_Name);
	  Doc_File_Name = Doc_ID + "_Live.TXT";
	  System.out.println("Doc_File_Name is " + Doc_File_Name);
	  Meta_File_Name = Doc_ID + "_Meta.csv";
	  System.out.println("Meta_File_Name is " + Meta_File_Name);
	  ...
	  try{
		  ...
		  for (FTPFile file : ftpFile) {
			  ...
			  if (details.equals(Audit_File_Name) ){
				  System.out.println("It matches the audit file.");
				  Audit_Match = true;
			  }
			  if (details.equals(Doc_File_Name)){
				  System.out.println("It matches the doc file.");
				  Doc_Match = true;
			  }
			  if (details.equals(Meta_File_Name)){
				  System.out.println("It matches the meta file.");
				  Meta_Match = true;
			  }
	  ...
	  Assert.assertFalse(Audit_Match, "The archive audit file name is exist.");
	  System.out.println("Pass. Audit file match.");
	  Assert.assertFalse(Doc_Match, "The archive doc file name is exist.");
	  System.out.println("Pass. Document file match.");
	  Assert.assertFalse(Meta_Match, "The archive meta file name is exist.");
	  System.out.println("Pass. Meta file match.");
	  Assert.assertFalse(Backup_Bool, "Backup file is true.");
	  System.out.println("Pass. Backup file is false.");
	  ...
	  //Verify the file has archived
	  driver.findElement(By.xpath("//*[contains(text(), 'Documents List')]")).click();
	  driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	  System.out.println("There is/are " + (driver.findElement(By.id("ctl00_cp1_tbl")).findElements(By.tagName("tr")).size() - 1) + " document/s after archive." );
	  Assert.assertTrue(Doc_List.size() > driver.findElement(By.id("ctl00_cp1_tbl")).findElements(By.tagName("tr")).size(), "After archive, it has the wrong number of doc on the list." );
	  ...
	  Assert.assertFalse(MySQL.Arch_DB.equals(null), "Arch is null.");
	  Assert.assertTrue(MySQL.Doc_ID_DB == Integer.parseInt(Doc_ID), "Doc ID does not match in database.");
